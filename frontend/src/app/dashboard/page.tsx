...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">
            Real-time ITSM operations overview
          </p>
        </div>
        <button
          onClick={refetch}
      ...
      {ticketStats.error && (
        <ErrorAlert
          message={`Ticket stats: ${ticketStats.error}`}
          onRetry={ticketStats.refetch}
        />
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <StatsCard
          ...
          value={employees.loading ? '...' : totalEmployees}
        ...
        <StatsCard
          ...
          value={integrations.loading ? '...' : activeIntegrations}
          subtitle={`of ${(integrations.data as any)?.length ?? 0} configured`}
          icon={Puzzle}
          color="green"
        />
        <StatsCard
          title="Open Tickets"
          value={ticketStats.loading ? '...' : (stats.open ?? 0)}
          subtitle={`${stats.critical ?? 0} critical`}
          icon={Ticket}
          color="amber"
        />
        <StatsCard
          title="AI Detected Issues"
          value={ticketStats.loading ? '...' : (stats.aiGenerated ?? 0)}
          subtitle="Auto-ticketed"
          icon={Bot}
          color="purple"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <StatsCard
          title="Total Tickets"
          value={ticketStats.loading ? '...' : (stats.total ?? 0)}
          subtitle="All time"
          icon={Ticket}
          color="blue"
        />
        <StatsCard
          ...
          value={ticketStats.loading ? '...' : (stats.resolved ?? 0)}
        ...
        <StatsCard
          ...
          value={ticketStats.loading ? '...' : (stats.critical ?? 0)}
          ...
          {provLogs.loading ? (
            <div className="flex justify-center py-8">
              <div className="h-5 w-5 animate-spin rounded-full border-2 border-slate-600 border-t-blue-500" />
                ...
                <ErrorAlert
                  message={provLogs.error}
                  onRetry={provLogs.refetch}
                />
              ) : logs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {logs.map((log: any) => (
                    <div
                      key={log._id}
                      className="flex items-start gap-3 pb-3 border-b border-slate-200 dark:border-slate-700 last:border-0 last:pb-0"
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-900 dark:text-slate-300 truncate">
                          ...
                          {log.provider} —{' '}
                          <span className="text-slate-500 dark:text-slate-400">{log.userEmail}</span>
                        </p>
                        <p className="text-xs text-slate-500 mt-0.5">
                          {new Date(log.createdAt).toLocaleString()}
                        </p>
                      </div>
                      <span className={statusVariantClass[log.status] || 'badge-info'}>
                        {log.status}
                      </span>
                    </div>
                  ))}
                      ...
                      <div className="flex items-center gap-3">
                        {integ.lastSyncAt && (
                          <span className="text-xs text-slate-500">
                            {new Date(integ.lastSyncAt).toLocaleTimeString()}
                          </span>
                        )}
                        <span
                          className={
                            integStatusClass[integ.status] || 'badge-info'
                          }
                        >
